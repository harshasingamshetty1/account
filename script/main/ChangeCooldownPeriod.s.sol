...
/// @title ChangeCooldownPeriod
/// @dev please never change the console.log the cli scripts parse these output and executes accordingly
/// @notice Script to change cooldown period in GardenSolver via hardware wallet signature
contract ChangeCooldownPeriod is Script {
    function run() public {
        address gardenSolverAddress = vm.envAddress("GARDEN_SOLVER");
        uint256 newCooldownPeriod = vm.envUint("COOLDOWN_PERIOD");
        ...
        console.log("CooldownDigest:", vm.toString(digest));
        ...
        console.log("\nBroadcasting cooldown period change transaction");
        vm.startBroadcast(deployerPrivateKey);
        solver.execute(calls, abi.encodePacked(nonce, signature));
        vm.stopBroadcast();

        console.log("Cooldown period changed!");
        console.log("New Cooldown Period:", newCooldownPeriod);
        console.log("New Cooldown Period (seconds):", newCooldownPeriod);
    }
