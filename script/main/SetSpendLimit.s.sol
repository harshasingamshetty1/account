...
/// @notice Script to set spend limit in GardenSolver via hardware wallet signature
contract SetSpendLimit is Script {
    function run() public {
        address gardenSolverAddress = vm.envAddress("GARDEN_SOLVER");
        address token = vm.envAddress("TOKEN_ADDRESS");
        uint256 spendLimit = vm.envUint("SPEND_LIMIT");
        ...
        console.log("SetSpendLimitDigest:", vm.toString(digest));
        ...
        console.log("\nBroadcasting spend limit transaction");
        vm.startBroadcast(deployerPrivateKey);
        solver.execute(calls, abi.encodePacked(nonce, signature));
        vm.stopBroadcast();

        console.log("Spend limit set!");
        console.log("KeyHash:", vm.toString(signerKeyHash));
        console.log("Token: address(0) (native)");
        console.log("Period: Forever");
        console.log("Limit:", spendLimit);
    }
